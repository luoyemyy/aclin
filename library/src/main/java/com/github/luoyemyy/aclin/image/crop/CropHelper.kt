...
class CropHelper(private val mImageView: ImageView) : PreviewHelper(mImageView) {
    ...
    private fun calculateCropSpace(): RectF {
        ...
        val w: Float
        val h: Float
        if (vRatio > mMaskRatio) {
            w = vh * mMaskRatio * mMaskPadding
            h = vh * mMaskPadding
        } else {
            w = vw * mMaskPadding
            h = vw / mMaskRatio * mMaskPadding
        }
        return RectF(vw / 2 - w / 2, vh / 2 - h / 2, vw / 2 + w / 2, vh / 2 + h / 2)
    }

    private fun checkBounds(): Boolean {
        val cr = calculateCropSpace() //缩放后-裁剪区域坐标
        val bitmap = (mImageView.drawable as? BitmapDrawable)?.bitmap ?: return true
        val br = RectF(0f, 0f, bitmap.width.toFloat(), bitmap.height.toFloat()).also {
            mImageView.imageMatrix.mapRect(it) //缩放后-图片区域坐标
        }
        return when {
            br.right - br.left < cr.right - cr.left -> false    //图片宽度小于裁剪区
            br.bottom - br.top < cr.bottom - cr.top -> false    //图片高度小于裁剪区
            br.left > cr.left -> false                          //裁剪区左边包含空白
            br.top > cr.top -> false                            //裁剪区上边包含空白
            br.right < cr.right -> false                        //裁剪区右边包含空白
            br.bottom < cr.bottom -> false                      //裁剪区下边包含空白
            else -> true
        }
    }
